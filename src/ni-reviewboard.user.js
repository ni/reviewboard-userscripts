...
// @require      https://gist.githubusercontent.com/alejandro5042/af2ee5b0ad92b271cd2c71615a05da2c/raw/easy-userscripts.js?v=70#sha384-jDwgCHbDuz3Crr4lWqnjJUbjVoZeouMp9v6zAHkV8DAPpiaXoO+O2LQJyJ9AHYeV
// ==/UserScript==
  ...
  eus.globalSession.onFirst(document, 'body', () => {
    ...
    if (reviewIdMatch) {
      ...
      eus.globalSession.onFirst(document, '#fieldset_reviewers_body', async (targetPeopleAndGroups) => {
        ...
        const reviewDataFetch = fetch(`https://review-board.natinst.com/api/review-requests/${reviewId}/reviews/`);
  ...
  eus.onUrl(/\/dashboard\//gi, (session, urlMatch) => {
    session.onFirst(document, '.datagrid-body', all => {
      if (!all.querySelector('colgroup .my_comments')) {
        swal.fire({
          html: '<p>To enable dashboard sorting, add the <code>My Comments</code> column to the table.</p>'
